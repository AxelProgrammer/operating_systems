    ...
    while True:
        try:
            ...
            if not message:
                ...
                break
            elif message.startswith('@'):  # Check if the message is a private message
                recipient, private_message = message.split(' ', 1)
                recipient = recipient[1:]  # Remove the '@' symbol
                if recipient in clients:
                    clients[recipient].send(bytes(f"{username} (private): {private_message}", 'utf-8'))
                else:
                    client_socket.send(bytes(f"User {recipient} is not online or does not exist.", 'utf-8'))
            else:
